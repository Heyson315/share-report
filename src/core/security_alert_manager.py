...
class SecurityAlertManager:
    ...
    def collect_alerts(self) -> int:
        ...
        self.alerts.sort(
            key=lambda a: self.severity_weights.get(a.severity, 0),
            reverse=True
        )
    ...
    def generate_remediation_log(self) -> Path:
        ...
        log_data = {
            ...
            "investigations": {
                aid: asdict(inv) for aid, inv in self.investigations.items()
            },
            "remediations": {
                aid: asdict(rem) for aid, rem in self.remediations.items()
            }
        }
    ...
    def generate_summary_report(self) -> Path:
        ...
        summary = {
            ...
            "actions_taken": [
                asdict(rem) for rem in self.remediations.values() if rem.success
            ],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="M365 Security Alert Investigation and Remediation Manager"
    )
    parser.add_argument(
        "--audit-file",
        type=Path,
        required=True,
        help="Path to M365 CIS audit JSON file"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--apply-remediation",
        action="store_true",
        help="Apply actual remediations (default is dry-run mode)"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    if dry_run:
        ...
        print(f"   Use --apply-remediation to apply changes")
