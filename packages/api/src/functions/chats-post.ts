...
export async function postChats(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    ...
    const sessionId = ((chatContext as ChatContext)?.sessionId) || uuidv4();
    ...
    if (uploadTextExecutionMode === 'evaluateInline') {
      ...
      // Clear the cache if the protectionScopeState is modified so that we can invoke the protection scope
      if (processContentJSONResponse?.protectionScopeState === 'modified') {
        context.log(`Clear the ETAG cache`);
        // TODO: If cache clearing is required for future policy changes, uncomment the next line.
