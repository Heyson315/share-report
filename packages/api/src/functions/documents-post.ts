...
export async function postDocuments(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    ...
    const fileField = parsedForm.get('file');
    if (!fileField || typeof fileField !== 'object' || !('arrayBuffer' in fileField)) {
      return badRequest('Uploaded file is invalid or missing required methods.');
    }
    // Azure Functions FormData returns a Blob-like object, but may not have 'name'
    const file: Blob & { name?: string } = fileField as Blob & { name?: string };
    const filename = file.name || `uploaded-${Date.now()}.pdf`;
        ...
        try {
          ...
          if (documentsToDelete && documentsToDelete.length > 0) {
            for (const document of documentsToDelete) {
              await container.item(document.id, document.id).delete();
            }
