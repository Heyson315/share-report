...
export function getUserId(request: HttpRequest, body?: unknown): string | undefined {
  ...
  if (
    ...
    body !== null &&
    'context' in body &&
    typeof (body as any).context === 'object' &&
    (body as any).context !== null &&
    'userId' in (body as any).context
  ) {
    userId ??= (body as { context: { userId: string } }).context.userId;
  }
