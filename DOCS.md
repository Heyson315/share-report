# 🔍 Quick Documentation Access

## 🎯 I Want To...

### **Get Started**
- **📘 Understand the project**: [README.md](README.md)
- **🏢 Development environment**: [docs/CPA_DEVELOPMENT_ENVIRONMENT.md](docs/CPA_DEVELOPMENT_ENVIRONMENT.md)
- **🚀 Set up development**: [PROJECT_OUTLINE.md](PROJECT_OUTLINE.md)
- **👥 Contribute code**: [CONTRIBUTING.md](CONTRIBUTING.md)
- **🌳 Git workflow**: [docs/GIT_BRANCH_STRATEGY.md](docs/GIT_BRANCH_STRATEGY.md)

### **AI Development**
- **🧠 AI agent development**: [.github/copilot-instructions.md](.github/copilot-instructions.md) ⭐ **START HERE**
- **🤖 MCP server extension**: [src/extensions/mcp/README.md](src/extensions/mcp/README.md)
- **📖 MCP development guide**: [docs/CUSTOM_MCP_SERVER_GUIDE.md](docs/CUSTOM_MCP_SERVER_GUIDE.md)

### **Security & Compliance**
- **🛡️ Run security audits**: [docs/SECURITY_M365_CIS.md](docs/SECURITY_M365_CIS.md)
- **📋 CIS compliance**: [PROJECT_OUTLINE.md](PROJECT_OUTLINE.md) (compliance section)

### **SharePoint Analysis**
- **📊 Analyze permissions**: [docs/USAGE_SHAREPOINT.md](docs/USAGE_SHAREPOINT.md)
- **🧹 Clean CSV data**: [scripts/README.md](scripts/README.md)

### **Automation & Deployment**
- **⚙️ Service principal setup**: [docs/M365_SERVICE_PRINCIPAL_SETUP.md](docs/M365_SERVICE_PRINCIPAL_SETUP.md)
- **🚀 Production deployment**: [docs/PRODUCTION_DEPLOYMENT.md](docs/PRODUCTION_DEPLOYMENT.md)
- **🔄 CI/CD setup**: [.github/workflows/README.md](.github/workflows/README.md)

### **Scripts & Commands**
- **⚡ PowerShell scripts**: [scripts/README.md](scripts/README.md)
- **🐍 Python tools**: [scripts/README.md](scripts/README.md)

---

## 📚 Complete Documentation

**📁 [Full Documentation Index](docs/README.md)** - Organized by role and use case

---

> **🧠 AI Agents**: Read [.github/copilot-instructions.md](.github/copilot-instructions.md) first for complete project architecture and patterns!
